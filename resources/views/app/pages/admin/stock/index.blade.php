    ...
    <div class="mx-auto max-w-screen-2xl">
        <div class="bg-white dark:bg-gray-800 relative shadow-md sm:rounded-lg">
            ...
            <div class="flex flex-col md:flex-row items-center justify-between space-y-3 md:space-y-0 md:space-x-4 p-4 border-t dark:border-gray-700">
                 <form action="{{ route('admin.stock.report') }}" method="GET" class="w-full md:w-1/2">
                    <div class="relative">
                        ...
                        {{-- PERBAIKAN: Menambahkan semua kelas dark mode pada input --}}
                        <input type="text" name="search" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg block w-full pl-10 p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" placeholder="Cari produk" value="{{ request('search') }}">
                    </div>
                </form>
                <div class="w-full md:w-auto flex flex-shrink-0 items-center space-x-3">
            ...
            <div class="overflow-x-auto">
                {{-- PERBAIKAN: Mengubah warna teks default tabel --}}
                <table class="w-full text-sm text-left text-gray-600 dark:text-gray-300">
                    ...
                    <tbody>
                        @forelse ($products as $product)
                            {{-- PERBAIKAN: Menambahkan hover effect yang konsisten --}}
                            <tr class="border-b dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                                ...
                                {{-- PERBAIKAN: Membuat angka stok lebih menonjol di dark mode --}}
                                <td class="px-4 py-3 font-bold text-gray-900 dark:text-white">{{ $product->stock }}</td>
                                <td class="px-4 py-3">{{ $product->minimum_stock }}</td>
                                <td class="px-4 py-3">
                                    @if ($product->stock == 0)
                                        <span class="bg-red-100 text-red-800 text-xs font-medium px-2.5 py-0.5 rounded-full dark:bg-red-900 dark:text-red-300">Habis</span>
                                    @elseif ($product->stock <= $product->minimum_stock)
                                        <span class="bg-yellow-100 text-yellow-800 text-xs font-medium px-2.5 py-0.5 rounded-full dark:bg-yellow-900 dark:text-yellow-300">Menipis</span>
                                    @else
                                        <span class="bg-green-100 text-green-800 text-xs font-medium px-2.5 py-0.5 rounded-full dark:bg-green-900 dark:text-green-300">Tersedia</span>
                                    @endif
                                </td>
                            </tr>
                        @empty
