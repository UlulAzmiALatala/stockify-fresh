                ...
                <div class="grid gap-4 mb-4 grid-cols-1 sm:grid-cols-2">
                    ...
                    <div>
                        ...
                        <input type="text" name="name"
                            ...
                            placeholder="Contoh: Laptop ProBook 14" value="{{ old('name') }}" required>
                        @error('name')
                    ...
                    <div>
                        ...
                        <input type="text" name="sku"
                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg block w-full p-2.5 dark:bg-gray-700"
                            placeholder="Contoh: LP-PB-14-001" value="{{ old('sku') }}" required>
                        @error('sku')
                            <p class="mt-1 text-xs text-red-500">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <input type="number" name="purchase_price"
                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg block w-full p-2.5 dark:bg-gray-700"
                            placeholder="10000000" value="{{ old('purchase_price') }}" required>
                    ...
                    <div>
                        ...
                        <input type="number" name="selling_price"
                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg block w-full p-2.5 dark:bg-gray-700"
                            placeholder="12500000" value="{{ old('selling_price') }}" required>
                        @error('selling_price')
                            <p class="mt-1 text-xs text-red-500">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        <label for="minimum_stock"
                            class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Stok Minimum</label>
                        ...
                        <select name="attributes[]" multiple
                            ...
                            @foreach ($attributes as $attribute)
                                <option value="{{ $attribute->id }}"
                                    {{ collect(old('attributes'))->contains($attribute->id) ? 'selected' : '' }}>
                                    {{ $attribute->name }}
                                </option>
                            @endforeach
                        </select>
                        <p class="text-xs text-gray-500 mt-1">Bisa pilih lebih dari satu atribut</p>
                        @error('attributes')
                            <p class="mt-1 text-xs text-red-500">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="sm:col-span-2">
                        ...
                        <textarea name="description" rows="4"
                            class="block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 dark:bg-gray-700"
                            placeholder="Tulis deskripsi produk di sini...">{{ old('description') }}</textarea>
                    </div>
                </div>

                <button type="submit"
                    class="text-white inline-flex items-center bg-primary-700 hover:bg-primary-800 focus:ring-4 focus:outline-none focus:ring-primary-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-primary-600 dark:hover:bg-primary-700">
