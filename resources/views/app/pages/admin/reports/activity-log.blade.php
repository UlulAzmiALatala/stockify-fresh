             ...
             <div class="flex flex-col md:flex-row items-center justify-between space-y-3 md:space-y-0 md:space-x-4 p-4 border-t dark:border-gray-700">
                <form action="{{ route('admin.reports.activity-log') }}" method="GET" class="w-full md:w-1/2">
                    <div class="relative">
                        ...
                        <input type="text" name="search" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg block w-full pl-10 p-2.5 dark:bg-gray-700" placeholder="Cari berdasarkan deskripsi..." value="{{ request('search') }}">
                    </div>
                </form>
            </div>
