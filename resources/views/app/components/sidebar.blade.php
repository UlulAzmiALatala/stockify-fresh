...
<aside id="logo-sidebar" class="fixed top-0 left-0 z-40 w-64 h-screen pt-20 transition-transform -translate-x-full bg-white border-r border-gray-200 sm:translate-x-0 dark:bg-gray-800 dark:border-gray-700" aria-label="Sidebar">
    ...
    <div class="h-full px-3 pb-4 overflow-y-auto bg-white dark:bg-gray-800">
        <ul class="space-y-2 font-medium">
            ...
            {{-- 1. Menu Dashboard --}}
            <li>
                {{-- KELAS TRANSISI DITAMBAHKAN DI SINI --}}
                <a href="{{ route('admin.dashboard') }}" class="flex items-center p-2 text-gray-900 rounded-lg dark:text-white hover:bg-indigo-500 dark:hover:bg-indigo-600 hover:text-white dark:hover:text-white group transition delay-150 duration-300 ease-in-out hover:-translate-y-1 hover:scale-110 {{ Request::is('admin/dashboard') ? 'bg-gray-100 dark:bg-gray-700' : '' }}">
                    <i class="w-6 text-center text-gray-500 fa-solid fa-chart-pie transition duration-75 group-hover:text-white dark:text-gray-400 dark:group-hover:text-white"></i>
